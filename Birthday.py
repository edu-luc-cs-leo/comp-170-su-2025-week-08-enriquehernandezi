...
class Birthday:
    ...
    #Mutator for month. It only changes the month value if valid (1-12)
        if month >= 1 and month <= 12:
            self.__month = month
    # end set_day
    ...
    def days_until(self):
        ...
        today_day = today.day #gets day
        today_month = today.month #gets month
        d_t = self.day_in_year(today_month, today_day) #d_t is day today and it computes todays date
        d_b = self.day_in_year(self.__month, self.__day) #d_b is day of birthday and computes the day of the person's birthday
        days = d_b - d_t #calculates how many days till birthday
        if days < 0: #if days is negative it means the birthday passed and so we add 365 days of the year
            days = 365 + days
